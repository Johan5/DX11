...
const SInputData& CDoubleBufferedInput::SwapBuffers()
{
	...
	static const std::vector<EInputCode> MouseInputCodes = { EInputCode::LeftMouseButton, EInputCode::RightMouseButton, EInputCode::MiddleMouseButton };
	auto IsMouseInput = []( EInputCode InputCode )
	{
		return std::find( MouseInputCodes.begin(), MouseInputCodes.end(), InputCode ) != MouseInputCodes.end();
	};
	...
	for ( const SKeyInput& KeyInput : _KeyInput[_FrontBufferIdx] )
	{
		...
		{
			bool IsNewInput = std::find( InputData._CurrentlyHeldDown.begin(), InputData._CurrentlyHeldDown.end(), KeyInput._InputCode ) == InputData._CurrentlyHeldDown.end();
			if ( IsNewInput )
			{
				InputData._CurrentlyHeldDown.push_back( KeyInput._InputCode );
				if ( IsMouseInput( KeyInput._InputCode ) )
				{
					InputData._NewMouseClicks.push_back( SMouseClick{ KeyInput._InputCode, KeyInput._CoordinateData } );
				}
				else
				{
