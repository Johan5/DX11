...
class CVector4f {
 public:
  ...
  CVector4f(const CVector3f& XYZ, float W)
...
inline CVector3f CVector3f::CalcCross(const CVector3f& Other) const {
  ...
  return CVector3f{_Y * Other._Z - _Z * Other._Y, _Z * Other._X - _X * Other._Z,
...
inline CVector4f CVector4f::operator/(float Denominator) const {
  return CVector4f{_X / Denominator, _Y / Denominator, _Z / Denominator,
                   _W / Denominator};
}
