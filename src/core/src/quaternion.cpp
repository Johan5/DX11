...
CQuaternion::CQuaternion(float S, const CVector3f& Vec) : _S(S), _Vec(Vec) {}
...
/* static */
CQuaternion CQuaternion::CreateRotationQuaternion(float AngleInRadians,
                                                  const CVector3f& UnitVector) {
  assert(NMiscMath::AlmostEqual(UnitVector.CalcLengthSquared(), 1.0f));
  return CQuaternion{cosf(AngleInRadians / 2.0f),
                     sinf(AngleInRadians / 2.0f) * UnitVector};
}