...
void CWorld::Initialize(CGraphics& Graphics, CInputHandler& InputHandler) {
  ...
  _Camera = std::make_unique<CCameraBase>(FieldOfView, AspectRatio);
  ...
  _LightConstantBuffer = Graphics.CreateConstantBuffer(
      sizeof(NWorldPass::SLightConstantBuffer), ECpuAccessPolicy::CpuWrite);
  _ShadowHandler.Initialize(Graphics, Graphics.GetScreenWidth(),
...
void CWorld::SpawnDefaultObjects(CGraphics& Graphics) {
  auto SpawnWall = [this](const CVector3f& Position, const CVector3f& Scale) {
    ...
    Object->AccessMaterialCb()._Material._TextureSlot = NGraphicsDefines::ShadowMapTextureSlot + 1;
  };
  ...
  Cube4->SetPosition(CVector3f{0.0f, -0.0f, 8.0f});

  auto* Char1 = SpawnGameObject<XBot>();
  Char1->SetPosition(CVector3f{0.0f, -1.0f, 2.0f});
}
...
void CWorld::PerCameraSetup(CRenderContext& RenderContext,
                            CCameraBase& Camera) {
  ...
  RenderContext.UpdateConstantBuffer(_CameraConstantBuffer, &CameraCb,
                                     sizeof(CameraCb));
  RenderContext.SetVertexShaderConstantBuffer(_CameraConstantBuffer,
                                              EConstantBufferIdx::PerCamera);
  RenderContext.SetPixelShaderConstantBuffer(_CameraConstantBuffer,
                                             EConstantBufferIdx::PerCamera);
}

void CWorld::PerLightSetup(CRenderContext& RenderContext, CLightSource& Light,
  ...
  RenderContext.UpdateConstantBuffer(_LightConstantBuffer, &LightCb,
                                     sizeof(LightCb));
  RenderContext.SetVertexShaderConstantBuffer(_LightConstantBuffer,
                                              EConstantBufferIdx::PerLight);
  RenderContext.SetPixelShaderConstantBuffer(_LightConstantBuffer,
                                             EConstantBufferIdx::PerLight);

  RenderContext.SetPixelShaderTexture(ShadowData._ShadowMap,
                                      NGraphicsDefines::ShadowMapTextureSlot);
  RenderContext.SetPixelShaderSampler(ShadowData._ShadowMapSampler,
                                      NGraphicsDefines::ShadowMapSamplerSlot);
}
...
void CWorld::RenderObjects(CRenderContext& RenderContext, CCameraBase& Camera) {
  ...
  _BatchRenderHelper.RenderInstanced(RenderContext, *_pGraphics,
                                     ERenderPass::Normal);
}
