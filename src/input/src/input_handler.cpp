...
CInputEvent::CInputEvent(const SInputData& InputData) : _InputData(InputData) {}

bool CInputEvent::IsPressed(EInputCode InputCode) const {
  return std::find(_InputData._CurrentlyHeldDown.begin(),
                   _InputData._CurrentlyHeldDown.end(),
                   InputCode) != _InputData._CurrentlyHeldDown.end();
}
...
CInputHandler::CInputHandler(int32_t GameWindowWidth, int32_t GameWindowHeight)
  ...
  ASSERT(_GameWindowWidth != 0.0f && _GameWindowHeight != 0.0f,
         "Incorrect window size, game will crash.");
}
...
void CInputHandler::RegisterKeyInputEventCallback(const void* pCallerId,
                                                  const KeyCbkFuncT& Callback) {
...
void CInputHandler::KeyInputFromOS(EInputType InputType, EInputCode InputCode) {
  ...
  if (InputType == EInputType::KeyDown || InputType == EInputType::KeyUp) {
    ...
    ASSERT(
        false,
        "Unrecognized input");  // not implemented anything else than key input yet
  }
}
