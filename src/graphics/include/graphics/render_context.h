...
class CRenderContext {
 public:
  ...
  void SetVertexShaderConstantBuffer(CConstantBuffer& ConstantBuffer,
                                     EConstantBufferIdx Index);
  void SetVertexShader(CVertexShader& VertexShader);
  void SetPixelShaderConstantBuffer(CConstantBuffer& ConstantBuffer,
                                    EConstantBufferIdx Index);
  void SetPixelShader(CPixelShader& PixelShader);
  void SetGeometryShader(CGeometryShader& GeometryShader);
  void SetGeometryShaderConstantBuffer(CConstantBuffer& ConstantBuffer,
                                       EConstantBufferIdx Index);
  ...
  void SetRenderTargets(int32_t NumTargets, ID3D11RenderTargetView** ppRTVs,
  ...
  void UpdateVertexBuffer(CVertexBuffer& VertexBuffer, const void* pNewData,
                          size_t NewDataSize);
  void UpdateIndexBuffer(CIndexBuffer& IndexBuffer, const void* pNewData,
                         size_t NewDataSize);
  // optimally, ConstantBuffer data should be 16 byte aligned
  void UpdateConstantBuffer(CConstantBuffer& ConstantBuffer,
                            const void* NewData, size_t NewDataSize);
