...
bool CGraphics::Initialize(int ScreenWidth, int ScreenHeight, HWND Wnd) {
  ...
  bool Result = _Direct3D->Initialize(
      ScreenWidth, ScreenHeight, NGraphicsDefines::IsVSyncEnabled, Wnd,
      NGraphicsDefines::IsFullScreen, NGraphicsDefines::ScreenDepth,
...
CVertexBuffer CGraphics::CreateVertexBuffer(
    const void* pVertexData, uint32_t VertexDataSizeInBytes,
...
CTexture CGraphics::CreateTexture(uint32_t Width, uint32_t Height,
                                  EGfxResourceDataFormat Format,
                                  uint32_t BindFlags,
                                  ECpuAccessPolicy AccessPolicy) {
...
CTexture CGraphics::CreateTexture(const D3D11_TEXTURE2D_DESC& TextureDesc) {
  ...
  HRESULT Result = AccessDevice().CreateTexture2D(&TextureDesc, nullptr,
...
CTexture CGraphics::CreateTexture(const D3D11_TEXTURE2D_DESC& TextureDesc,
  ...
  Microsoft::WRL::ComPtr<ID3D11Texture2D> pTexture;
  HRESULT Result = AccessDevice().CreateTexture2D(&TextureDesc, &pInitialData,
                                                  pTexture.GetAddressOf());
  ASSERT(SUCCEEDED(Result), "Failed to create texture resource");
  return CTexture{pTexture};
}
...
std::optional<CTexture> CGraphics::GetTextureByName(const std::string& Name) {
  ...
  return (it == _Textures.end()) ? std::nullopt
                                 : std::optional<CTexture>{it->second};
}

std::optional<CTextureView> CGraphics::GetTextureViewByName(
    const std::string& Name) {
  auto it = _TextureViews.find(Name);
  return (it == _TextureViews.end()) ? std::nullopt
                                     : std::optional<CTextureView>{it->second};
}
...
CTextureView CGraphics::CreateTextureView(
    CTexture& Texture, const D3D11_SHADER_RESOURCE_VIEW_DESC& Desc) {
  Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> pTextureView;
  HRESULT Result = AccessDevice().CreateShaderResourceView(
      Texture.AccessTexture(), &Desc, pTextureView.GetAddressOf());
  ASSERT(SUCCEEDED(Result), "Failed to create shader resource view");
  return CTextureView{pTextureView};
}

CRenderTargetView CGraphics::CreateRenderTargetView(
    CTexture& Texture, const D3D11_RENDER_TARGET_VIEW_DESC& Desc) {
  Microsoft::WRL::ComPtr<ID3D11RenderTargetView> pRenderTargetView;
  HRESULT Result = AccessDevice().CreateRenderTargetView(
      Texture.AccessTexture(), &Desc, pRenderTargetView.GetAddressOf());
  ASSERT(SUCCEEDED(Result), "Failed to create render target view");
  return CRenderTargetView{pRenderTargetView};
}

CDepthStencilView CGraphics::CreateDepthStencilView(
    CTexture& Texture, const D3D11_DEPTH_STENCIL_VIEW_DESC& Desc) {
  Microsoft::WRL::ComPtr<ID3D11DepthStencilView> pDepthStencilView;
  HRESULT Result = AccessDevice().CreateDepthStencilView(
      Texture.AccessTexture(), &Desc, pDepthStencilView.GetAddressOf());
  ASSERT(SUCCEEDED(Result), "Failed to create render target view");
...
CSamplerState CGraphics::CreateSamplerState(
  ...
  HRESULT Result = AccessDevice().CreateSamplerState(
...
CRenderContext& CGraphics::StartRenderFrame(const CVector4f& BackgroundColor) {
  _Direct3D->BeginScene(BackgroundColor._X, BackgroundColor._Y,
                        BackgroundColor._Z, BackgroundColor._W);
...
bool CGraphics::initShaders() {
  ...
  CubeInputLayout.push_back(SShaderInputDescription{
      "POSITION", EGfxResourceDataFormat::R32G32B32Float});
  CubeInputLayout.push_back(SShaderInputDescription{
      "NORMAL", EGfxResourceDataFormat::R32G32B32Float});
  CubeInputLayout.push_back(
      SShaderInputDescription{"TEXCOORD", EGfxResourceDataFormat::R32G32Float});
  CubeInputLayout.push_back(SShaderInputDescription{
      "SV_InstanceID", EGfxResourceDataFormat::R32UInt});
  std::optional<CVertexShader> cubeVs = CreateVertexShader(
      (shaderFolder / shader_names::DefaultVertexShaderFileName).string(),
      shader_names::DefaultVertexShaderMainFunction, CubeInputLayout);
  std::optional<CPixelShader> cubePs = CreatePixelShader(
      (shaderFolder / shader_names::DefaultPixelShaderFileName).string(),
      shader_names::DefaultPixelShaderMainFunction);
  if (cubeVs && cubePs) {
    _VertexShaders.emplace(shader_names::DefaultVertexShaderFileName,
                           cubeVs.value());
    _PixelShaders.emplace(shader_names::DefaultPixelShaderFileName,
                          cubePs.value());
  } else {
    return false;
  }
  std::optional<CVertexShader> cubeShadowVs = CreateVertexShader(
      (shaderFolder / shader_names::CubeShadowVertexShaderFileName).string(),
      shader_names::CubeShadowVertexShaderMainFunction, CubeInputLayout);
  std::optional<CPixelShader> cubeShadowPs = CreatePixelShader(
      (shaderFolder / shader_names::CubeShadowPixelShaderFileName).string(),
      shader_names::CubeShadowPixelShaderMainFunction);
  std::optional<CGeometryShader> cubeShadowGs = CreateGeometryShader(
      (shaderFolder / shader_names::CubeShadowGeometryShaderFileName).string(),
      shader_names::CubeShadowGeometryShaderMainFunction);
  if (cubeShadowVs && cubeShadowPs && cubeShadowGs) {
    _VertexShaders.emplace(shader_names::CubeShadowVertexShaderFileName,
                           cubeShadowVs.value());
    _PixelShaders.emplace(shader_names::CubeShadowPixelShaderFileName,
                          cubeShadowPs.value());
    _GeometryShaders.emplace(shader_names::CubeShadowGeometryShaderFileName,
                             cubeShadowGs.value());
  } else {
    return false;
  }
...
bool CGraphics::initTextures() {
  ...
  std::filesystem::path path =
      std::filesystem::current_path() / "assets" / "bricks.jpg";
  int32_t width;
  int32_t height;
  ...
  if (data) {
    ...
    TextureDesc.ArraySize = 1;
    TextureDesc.Format =
        static_cast<DXGI_FORMAT>(EGfxResourceDataFormat::R32G32B32A32Float);
    TextureDesc.SampleDesc = DXGI_SAMPLE_DESC{1, 0};
    ...
    CLogger::LogFormat("Failed to load texture. Path='{}', Error='{}'",
                       path.string(), error);
    return false;
  }
}
...
bool CGraphics::initMeshes() {
  _Meshes.emplace(EMeshType::Cube,
...
std::optional<CVertexShader> CGraphics::CreateVertexShader(
  ...
  bool Success = VertexShader.Initialize(AccessDevice(), ShaderFileName,
                                         ShaderMainFunction, ShaderInputLayout);
  if (Success) {
    return VertexShader;
  }
...
std::optional<CGeometryShader> CGraphics::CreateGeometryShader(
    const std::string& ShaderFileName, const std::string& ShaderMainFunction) {
...
std::optional<CPixelShader> CGraphics::CreatePixelShader(
    const std::string& ShaderFileName, const std::string& ShaderMainFunction) {
  CPixelShader PixelShader;
  bool Success = PixelShader.Initialize(AccessDevice(), ShaderFileName,
                                        ShaderMainFunction);
