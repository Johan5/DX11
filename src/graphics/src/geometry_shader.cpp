...
void CGeometryShader::Initialize(ID3D11Device& Device,
  ...
  Result = D3DCompileFromFile(
      FileNameW.c_str(), nullptr, nullptr, ShaderMainFunction.c_str(), "gs_5_0",
      D3D10_SHADER_ENABLE_STRICTNESS, 0, pCompiledGeometryShader.GetAddressOf(),
      pErrorMessage.GetAddressOf());

  if (FAILED(Result)) {
    if (pErrorMessage) {
      CLogger::LogFormat("Failed to compile shader file %s. Error: ",
                         FileName.c_str());
    } else {
  ...
  Result =
      Device.CreateGeometryShader(pCompiledGeometryShader->GetBufferPointer(),
                                  pCompiledGeometryShader->GetBufferSize(),
                                  nullptr, _pGeometryShader.GetAddressOf());
